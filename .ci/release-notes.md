...
* **New: the disc screen is back, and it reads out-of-region discs.** Press **Z** in the menu and the console's own Game Play screen opens, with the disc's cover art and **PRESS START** exactly as a stock GameCube shows them — including for **imports**, which the stock screen refuses to read. **B** returns to the menu. The screen itself is the console's; what changed is who reads the disc. The stock flow only reaches a banner by running the apploader first, which loads the game into the memory cubiboot occupies and takes the console down mid-read, so cubiboot reads `opening.bnr` off the disc itself instead — a step per frame, so "Reading disc…" animates over the drive's spin-up rather than freezing on the button press. The region check lives in the machine being skipped, which is why an import boots. Built on the disc-screen infrastructure from [PR #8](https://github.com/DarthMotzkus/cubiboot-new-ui/pull/8) by [@Jpe230](https://github.com/Jpe230), whose reverse-engineering of the stock screen across all seven IPL revisions is what made this possible. On a console with an **ODE** (GC Loader, CUBE-ODE) the screen does not open at all: the ODE occupies the drive connector, so there is no optical drive behind it to read from, and **Z** answers with the menu's error tone. A **FlippyDrive keeps the disc screen** — see the next note. See [Booting a physical disc](https://github.com/DarthMotzkus/cubiboot-new-ui#booting-a-physical-disc).

* **New: the FlippyDrive stopped being treated as an ODE — because it isn't one.** A GC Loader or CUBE-ODE takes the optical drive's place; a FlippyDrive rides the drive ribbon *beside* the drive, so the drive is still there. Everything now follows from that. **The disc screen works on a FlippyDrive**: cubiboot switches the drive into bypass for the read and back afterwards, so **Z** opens the Game Play screen for the disc in the tray (a console whose drive was removed just gets the stock no-disc sequence). **In-Game Reset needs nothing installed**: the drive autoloads cubiboot from its flash on every reboot, so cubiboot now passes Swiss `IGRType=Reboot` on this hardware — `apploader.img` is not used on a FlippyDrive and never worked there; if you copied one to `swiss/patches/`, it is simply ignored. **`swiss-gc.dol` on the card became optional**: Swiss ships in the drive's flash and cubiboot uses that copy first, so the card-root copy is only a precaution. And in `device_order`, `ode` now means the two real ODEs only, while the FlippyDrive answers to its own `flippy` entry (present in the default order, so nothing changes if you never set the key). The menu header now says **FLIPPY SD** instead of ODE SD, too.
...
* **New: the menu header names the device you are reading from.** At the card root the header reads **SD2SP2**, **SLOT A SD**, **SLOT B SD**, **ODE SD** or **FLIPPY SD** instead of the product name, so a console with more than one card says which one the list came from. Inside a folder it still shows the folder name.
