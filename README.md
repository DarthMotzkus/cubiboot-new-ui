...
the drive's **flash** — cubiboot prefers that copy, and a `swiss-gc.dol` at the card root is
only the fallback. And In-Game Reset needs **no `apploader.img`**: the drive autoloads cubiboot
from flash on every reboot, so a plain reboot already lands back on the menu. Set Swiss's
**In-Game Reset** to **`Reboot`** (Settings → Global Game Settings) — cubiboot passes that
automatically for games it boots, so the setting only matters for games you start from inside
Swiss yourself.
